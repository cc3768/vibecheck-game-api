...
async function analyzeActionNote(note: string, intent: string, nearbyTerrain: string[], nearbyObjects: string[], character: CharacterRecord): Promise<ActionAnalysis | null> {
  if (!note.trim()) return null;
  try {
    const analysis = await serviceFetch<{ result?: { proposals?: Array<Record<string, unknown>>; confidence?: number; target?: { text?: string; normalizedKey?: string; type?: string }; objective?: string; broadSearch?: boolean; canCreateContent?: boolean; searchTags?: string[]; observationBias?: string; targetType?: string; targetKey?: string; message?: string } }>("ai-system", "/api/v1/ai/analyze-action", "POST", {
      note,
      intent,
      nearbyTerrain,
      nearbyObjects,
    ...
    return {
      objective: String(result.objective ?? (intent === "SCOUT" ? "SURVEY_AREA" : "GENERAL")),
      target: result.target ?? null,
      targetType: result.targetType ? String(result.targetType) : null,
      targetKey: result.targetKey ? String(result.targetKey) : null,
      broadSearch: Boolean(result.broadSearch),
      canCreateContent: Boolean(result.canCreateContent),
      searchTags: Array.isArray(result.searchTags) ? result.searchTags.map((entry) => String(entry)) : [],
      observationBias: String(result.observationBias ?? "survey"),
      confidence: Number(result.confidence ?? 0),
      proposals: Array.isArray(result.proposals) ? result.proposals : [],
      message: result.message ? String(result.message) : undefined
    } satisfies ActionAnalysis;
  } catch {
    return null;
  }
}

async function discoverContent(note: string, intent: string, nearbyTerrain: string[], nearbyObjects: string[], character: CharacterRecord, analysis?: ActionAnalysis | null): Promise<DiscoveryRecord | null> {
  if (!note.trim()) return null;
  try {
    const resolvedAnalysis = analysis ?? await analyzeActionNote(note, intent, nearbyTerrain, nearbyObjects, character);
    const proposals = resolvedAnalysis?.proposals ?? [];
    if (!resolvedAnalysis?.canCreateContent || !proposals.length) return null;
    const resolved = await serviceFetch<{ content?: DiscoveryRecord }>("creation-system", "/api/v1/creation/resolve-proposals", "POST", {
      ...
      knownSkills: character.skills.map((entry) => entry.skill),
      knownItems: Object.keys((character as CharacterRecord & { inventory?: Record<string, number> }).inventory ?? {}),
      broadSearch: resolvedAnalysis.broadSearch,
      canCreateContent: resolvedAnalysis.canCreateContent,
      objective: resolvedAnalysis.objective,
      targetKey: resolvedAnalysis.targetKey,
      proposals
    });
...
const GENERIC_HERB_HINTS = ["herb", "herbs", "moonmint", "sunroot", "silverleaf", "bitterwort", "pine needle", "pine needles", "mint"];

function herbLikeItemKey(itemKey: string) {
  const normalized = normalizeTextLocal(itemKey);
  return normalized === "unidentified_herb" || normalized.startsWith("herb_") || GENERIC_HERB_HINTS.some((hint) => normalized.includes(hint));
}

function shouldCollapseToUnidentifiedHerb(itemKey: string, plan: ActionPlan) {
  if (herbLikeItemKey(itemKey)) return true;
  if (plan.searchTags.includes("herb")) return true;
  const normalized = normalizeTextLocal(plan.note ?? "");
  return /(herb|moonmint|sunroot|silverleaf|bitterwort|pine needle|mint)/.test(normalized);
}

function normalizeGatheredItemKey(itemKey: string, plan: ActionPlan) {
  if (!itemKey) return itemKey;
  return shouldCollapseToUnidentifiedHerb(itemKey, plan) ? "unidentified_herb" : itemKey;
}
...
async function buildPlan(character: CharacterRecord, rawAction: ActionDraft): Promise<ActionPlan> {
  ...
  let objective = intent === "SCOUT" ? "SURVEY_AREA" : "GENERAL";
  let broadSearch = false;
  let canCreateContent = false;
  let searchTags: string[] = [];
  let observationBias = "survey";
  ...
  if (["OBSERVE", "SCOUT", "FORAGE", "MINE", "CRAFT_RECIPE", "BUILD"].includes(intent) && draft.note) {
    const analysis = await analyzeActionNote(draft.note, intent, nearbyTerrain, nearbyObjects, character);
    objective = analysis?.objective ?? objective;
    broadSearch = Boolean(analysis?.broadSearch);
    canCreateContent = Boolean(analysis?.canCreateContent);
    searchTags = analysis?.searchTags ?? [];
    observationBias = analysis?.observationBias ?? observationBias;
    analysisConfidence = Number(analysis?.confidence ?? 0);
    discovery = await discoverContent(draft.note, intent, nearbyTerrain, nearbyObjects, character, analysis);
...
type SearchOutcome = {
  mode: "none" | "observation" | "signal" | "item";
  itemKey?: string;
  amount?: number;
  message: string;
};

const TERRAIN_SEARCH_POOLS: Record<string, { collectibles: Array<{ itemKey: string; weight: number; message: string }>; observations: string[]; signals: string[] }> = {
  forest: {
    collectibles: [
      { itemKey: "unidentified_herb", weight: 3, message: "You notice a patch of unfamiliar herbs half-hidden in the undergrowth." },
      { itemKey: "bitter_root", weight: 2, message: "A bitter root shows where the soil has been turned recently." },
      { itemKey: "stem_fiber", weight: 2, message: "You strip usable fiber from a rough cluster of stalks." },
      { itemKey: "strange_seed", weight: 1, message: "You spot a strange seed pod tucked under the brush." }
    ],
    observations: [
      "You read the brush and note game paths, old foraging signs, and places worth checking later.",
      "The undergrowth shifts with small life, but nothing comes easily into hand this pass.",
      "You spot several promising patches, though most will take a more focused search to work properly."
    ],
    signals: [
      "You catch signs of a richer herb patch deeper in the brush.",
      "You notice broken stems and disturbed earth that suggest something harvestable nearby.",
      "A faint resin smell and bent growth hint that useful materials are close, but not obvious yet."
    ]
  },
  rock: {
    collectibles: [
      { itemKey: "flint", weight: 3, message: "You spot dark flint among the broken stone and pry a usable piece free." },
      { itemKey: "stone_chunk", weight: 3, message: "A loose stone seam gives way to a workable chunk." },
      { itemKey: "quartz", weight: 1, message: "A pale glint catches the light — enough quartz to carry away." },
      { itemKey: "shale_fragment", weight: 2, message: "You scrape free a brittle shale fragment from the surface." }
    ],
    observations: [
      "You trace fracture lines in the rock and mark a few spots worth striking later.",
      "Most of what you find is poor stone, but the seam is not dead yet.",
      "You study the ground and learn where the rock changes color and density."
    ],
    signals: [
      "A narrow seam suggests better stone if you come back with a more deliberate strike.",
      "You notice a glassy glint in the rock face, but it will take a targeted effort to free it.",
      "The stone here rings differently in one patch, suggesting a better vein nearby."
    ]
  },
  water: {
    collectibles: [
      { itemKey: "fresh_water", weight: 3, message: "You find a cleaner pocket of water and fill what you can carry." },
      { itemKey: "clay_lump", weight: 2, message: "Soft clay sits under the wet bank, ready to be scooped free." },
      { itemKey: "reeds", weight: 2, message: "A stand of reeds proves easy to cut and bundle." }
    ],
    observations: [
      "You follow the waterline and spot places where useful material gathers after flow and rain.",
      "The bank shifts underfoot, but you learn where the ground holds firmer and what collects there.",
      "You watch the edge of the water and notice signs of clay and reeds building in pockets."
    ],
    signals: [
      "A silty pocket nearby looks rich enough to work for clay later.",
      "You notice denser reeds farther down the waterline.",
      "The water has left a strip of debris that may hide useful material on another pass."
    ]
  },
  grass: {
    collectibles: [
      { itemKey: "bitter_root", weight: 2, message: "You pull a bitter root free from the dry grass." },
      { itemKey: "stem_fiber", weight: 2, message: "You gather stripped grass fiber that can still be put to use." },
      { itemKey: "strange_seed", weight: 1, message: "A dry seed cluster catches on your hand as you search the grass." },
      { itemKey: "flint", weight: 1, message: "You notice a hard flint fragment among the trampled ground." }
    ],
    observations: [
      "You sweep the grassland and mark where roots, seeds, and loose stone tend to collect.",
      "Wind and old foot traffic make the ground hard to read, but a few patterns stand out.",
      "You do not take anything this time, but you learn where the land gathers useful scraps."
    ],
    signals: [
      "You notice a denser patch of growth that could hide useful roots on a better pass.",
      "Loose debris in the grass suggests something workable is being exposed here over time.",
      "The ground is ordinary at first glance, but one patch looks better for gathering than the rest."
    ]
  }
};
...
function searchPoolFor(plan: ActionPlan) {
  const terrainOrder = [...plan.nearbyTerrain, "grass", "forest", "rock", "water"];
  for (const terrain of terrainOrder) {
    const pool = TERRAIN_SEARCH_POOLS[terrain];
    if (pool) return pool;
  }
  return TERRAIN_SEARCH_POOLS.grass;
}

function resolveBroadSearch(plan: ActionPlan): SearchOutcome {
  const pool = searchPoolFor(plan);
  const itemBias = plan.objective === "FIND_COLLECTIBLES" ? 0.42 : 0.22;
  const signalBias = plan.objective === "FIND_COLLECTIBLES" ? 0.26 : 0.24;
  const roll = Math.random();
  if (roll < itemBias) {
    const filtered = pool.collectibles.filter((entry) => {
      if (!plan.searchTags.length) return true;
      if (plan.searchTags.includes("mineral")) return ["flint", "stone_chunk", "quartz", "shale_fragment", "clay_lump"].includes(entry.itemKey);
      if (plan.searchTags.includes("plant") || plan.searchTags.includes("collectible")) return ["unidentified_herb", "bitter_root", "stem_fiber", "strange_seed", "reeds"].includes(entry.itemKey);
      if (plan.searchTags.includes("water")) return ["fresh_water", "clay_lump", "reeds"].includes(entry.itemKey);
      return true;
    });
    const choice = weightedPick(filtered.length ? filtered : pool.collectibles);
    if (choice) return { mode: "item", itemKey: choice.itemKey, amount: 1, message: choice.message };
  }
  if (roll < itemBias + signalBias) {
    return { mode: "signal", message: randomPick(pool.signals) ?? "You notice signs that something useful may be nearby, but not enough to take yet." };
  }
  if (roll < itemBias + signalBias + 0.25) {
    return { mode: "observation", message: randomPick(pool.observations) ?? "You study the area and come away with a better read of the land." };
  }
  return { mode: "none", message: "You search carefully, but this pass turns up nothing you can take with confidence." };
}
...
function resolveScout(character: CharacterRecord, plan: ActionPlan) {
  ...
  if (plan.broadSearch || !plan.canCreateContent) {
    const broadResult = success ? resolveBroadSearch(plan) : { mode: "none" as const, message: "You search carefully but fail to spot anything worth acting on." };
    if (broadResult.itemKey && broadResult.amount) inventoryChanges[broadResult.itemKey] = broadResult.amount;
    return {
      category: plan.category,
      intent: plan.intent,
      success,
      ...
      discoveredSkills: [],
      message: broadResult.message
    } satisfies ActionOutcome;
  }

  let message = "You search carefully but fail to spot anything worth keeping.";
  if (success) {
    const found = normalizeGatheredItemKey(plan.discovery?.item?.itemKey ?? (plan.nearbyTerrain.includes("forest") ? "unidentified_herb" : plan.nearbyTerrain.includes("water") ? "fresh_water" : "strange_seed"), plan);
...
function resolveForage(character: CharacterRecord, plan: ActionPlan) {
  ...
  if (success) {
    ...
    const found = normalizeGatheredItemKey(plan.discovery?.item?.itemKey ?? (normalized.includes("berry") ? "wild_berry" : normalized.includes("root") ? "bitter_root" : normalized.includes("herb") ? "unidentified_herb" : plan.nearbyTerrain.includes("forest") ? "unidentified_herb" : "bitter_root"), plan);
    inventoryChanges[found] = 1;
    return {
      category: plan.category,
      intent: plan.intent,
...
function resolveUnpackHerbBundle() {
  const inventoryChanges: Record<string, number> = { herb_bundle: -1, unidentified_herb: 3, stem_fiber: 1 };
  return {
    ...
    drawbacks: [],
    itemMeta: registerKnownMeta(inventoryChanges),
    message: "You untie the bundle and sort the contents into unidentified herbs and usable fiber."
  } satisfies ActionOutcome;
}
...
app.post("/api/v1/actions/explain", async (req, res) => {
  const requestId = getRequestId(req);
  const characterId = String(req.body.characterId ?? "");
  if (!characterId) return sendError(res, SERVICE_NAME, SERVICE_VERSION, requestId, "MISSING_CHARACTER", "characterId is required", 400);
  try {
    const data = await serviceFetch<{ character: CharacterRecord | null }>("character-system", `/api/v1/character/${characterId}`);
    if (!data.character) return sendError(res, SERVICE_NAME, SERVICE_VERSION, requestId, "CHARACTER_NOT_FOUND", "Character not found", 404);
    const plan = await buildPlan(data.character, (req.body.action ?? {}) as ActionDraft);
    sendSuccess(res, SERVICE_NAME, SERVICE_VERSION, requestId, {
      understanding: {
        intent: plan.intent,
        category: plan.category,
        objective: plan.objective,
        broadSearch: plan.broadSearch,
        canCreateContent: plan.canCreateContent,
        searchTags: plan.searchTags,
        observationBias: plan.observationBias,
        requiredSkills: plan.requiredSkills,
        allowed: plan.allowed,
        reasons: plan.reasons,
        discoveryMessage: plan.discovery?.message ?? null
      }
    });
  } catch (error) {
    sendError(res, SERVICE_NAME, SERVICE_VERSION, requestId, "ACTION_EXPLAIN_FAILED", error instanceof Error ? error.message : "Action explain failed", 500);
  }
});
